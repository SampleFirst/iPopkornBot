...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
            ...
            if userid in ADMINS:
                buttons = [
                    ...
                    [
                        ...
                        InlineKeyboardButton('Manual Start' if settings["botpm"] else 'Channel', callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}')
                    ],
                    [
        ...
        buttons = [
            ...
            [
                InlineKeyboardButton('More Bots', url=BOTS_CHANNEL),
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = message.text
            if isinstance(search, str):  # Check if search is a string
